...
class MsprimeSimulator(DataSimulator):
    ...
    def run(self, rep: int = None, seed: int = None) -> list[dict[str, str]]:
        ...
        if self.nsrc is not None and self.nsrc > 0:
            samples.append(
                msprime.SampleSet(self.nsrc, ploidy=self.ploidy, population=self.src_id)
            )
        if self.out_id is not None and self.nout > 0:
            samples.append(
                msprime.SampleSet(self.nout, ploidy=self.ploidy, population=self.out_id)
            )
    ...
    def _create_ref_tgt_file(
        ...
        if src_ind_file:
            with open(src_ind_file, "w") as f:
                for i in range(nref + ntgt, nref + ntgt + nsrc):
                    f.write(identifier + str(i) + "\n")

        if out_ind_file:
            if not nsrc:
                nsrc = 0
            with open(out_ind_file, "w") as f:
                for i in range(nref + ntgt + nsrc, nref + ntgt + nsrc + nout):
                    f.write(identifier + str(i) + "\n")
