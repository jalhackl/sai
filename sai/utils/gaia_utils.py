...
def parse_ind_file(filename):
    ...
    Description:
        ...
        Assumes that the last whitespace-separated token in each line is the sample name.

    Arguments:
        filename str: Name of the file containing sample information.

    Returns:
        samples list: List of sample identifiers (last word in each line).
    """
...
def read_data_src(vcf_file, ref_ind_file, tgt_ind_file, src_ind_file, anc_allele_file, is_phased, filter_fixed=False):
    """
    Description:
        Helper function for reading data from reference and target populations.
    ...
    ref_data = ref_samples = tgt_data = tgt_samples = src_samples = None
    if ref_ind_file is not None:
        ref_samples = parse_ind_file(ref_ind_file)
        ref_data = read_geno_data(vcf_file, ref_samples, anc_allele_file, True)
    ...
    chr_names = tgt_data.keys()
    if filter_fixed:
        if (ref_ind_file is not None) and (tgt_ind_file is not None) and (src_ind_file is not None):
            ...
            for c in chr_names:
                ...
                #src
                src_fixed_variants = np.sum(src_data[c]["GT"].is_hom_alt(), axis=1) == len(
                    src_samples
                )

                fixed_index = np.logical_and(np.logical_and(ref_fixed_variants, tgt_fixed_variants), src_fixed_variants)
                index = np.logical_not(fixed_index)
                # fixed_pos = ref_data[c]["POS"][fixed_index]
                ref_data = filter_data(ref_data, c, index)
                tgt_data = filter_data(tgt_data, c, index)
